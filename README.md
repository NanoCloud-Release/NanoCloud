...
- 有手把手教授的详细文档
- 官网直连，无需代理即可获取配置。
- VLESS节点 ws + TLS + CDN，未跳过证书验证，能防止 MIMT 攻击。隐私性更好，内容数据不会被机场读取*。
- TUIC 和 hy2 节点 —— 自签名直连节点，伪装bing.com，跳过证书验证，内容数据证书因为加密不会被偷。
- mieru 节点 —— 一种适用于现在环境的方案

*不管外层用不用正规证书，由于流量通过了机场主的 VPS 节点，机场主在任何情况下都能看到以下两类信息：
1. 你访问了什么域名（SNI 与目标 IP）：机场主知道你在什么时间访问了 youtube.com 或 github.com，因为他需要负责把数据包路由到目标服务器。
2. 未经加密的明文 HTTP 流量：如果你访问的是古老的、没有小锁的 http:// 纯明文网站，机场主可以在 VPS 落地端直接截获明文内容。
...
> **优惠码：`KFZQWM`**
> 有效期：**2026 年 9 月 4 日至 9 月 9 日**（含首尾日期）
> 年付专属 **85 折**，适用于白羊座、双鱼座和射手座套餐。

购买时将付款周期切换为“年付”，在订单中填写 `KFZQWM`，并确认页面显示折扣后再提交。优惠码不适用于月付。
...
安装 NanoCloud 官方客户端并按教程导入证书，可以看到更多节点。以当前记录为例，猎户座开源客户端可导入 27 个节点，官方客户端有 54 个；射手座开源客户端有 58 个，官方客户端有 88 个。不同套餐的订阅内容会变化，数字只作当时参考。官方客户端支持 Android、Windows 和 macOS，使用 Mihomo 内核，这一点与 Clash Verge 相同。奇怪的是写配置的时候拉取的节点不知为何就是不一样
...
## 怎么选客户端
看需求。如果你要使用对 IP 质量敏感的 ChatGPT Pro 或 Claude Max，并打算自购商业 ISP 作为链式代理，就需要开源客户端或直接使用内核。官方客户端没有提供这种配置入口。
想要更稳定和更多节点，用官方客户端。当开源客户端拉取的节点不好用的时候，比如特殊时期封锁加重，推荐使用官方客户端。
...
已有猎户座、白羊座、双鱼座或射手座套餐的用户，也可以使用优惠码购买或升级年付订阅。更换套餐时请同时参考下方的抵扣规则。

如果流量需求不大，可以和朋友合购一个白羊座、双鱼座或射手座年付套餐。请自行管理账号和订阅地址，避免将订阅分享给无关人员；同时在线设备上限分别为白羊座 5 台、双鱼座 8 台、射手座 10 台。

### 续费与换购

- **同套餐续费：** 如果剩余流量较多，套餐到期时间顺延，当前流量保留到下一个账单日再重置；如果剩余流量很少或已经用完，系统会重置已用流量，并从续费当天重新计算套餐时间。
- **更换套餐：** 系统自动抵扣原套餐剩余价值，并收取 15% 手续费后计入退款余额。已用流量低于 10% 时按 30 天折算一个月，高于或等于 10% 时按 31 天折算，月份取整。

例如，小明 10 月 1 日购买白羊座，10 月 25 日续费一个月白羊座。如果此时已用 200G、剩余 100G，套餐到期时间顺延 31 天，流量在下个套餐生效时重置；如果已用流量达到 280G 或已经用完，则流量立即重置，套餐时间从 10 月 25 日重新计算。

再如猎户座年付 ¥12，使用不到 10%，还剩 364 天：`364 ÷ 30 = 12` 个整月，抵扣金额为 `12 × ¥1 × 85% = ¥10.20`。换购 ¥10 的白羊座后，退款余额为 ¥0.20。

当前购买页只显示月付，年付按 12 个月的月付价格结算。
新年优惠码已过期。
劳动节优惠码 `MAYDAY85` 已过期。
618 优惠码 `OFF61885` 过期
...
| 使用方式 | 支持平台或客户端 | 特点 |
| --- | --- | --- |
| NanoCloud 官方客户端 | Android、Windows、macOS | 配置省事，节点更完整；猎户座 55 个，射手座 88 个 |
| 控制台一键导入 | Hiddify Next、Clash Verge、FlClash、Karing、sing-box | 继续使用熟悉的客户端和自定义配置；猎户座 27 个，射手座 38 个 |
